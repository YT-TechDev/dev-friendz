...
The stable product sequence is:

```text
v0.1.0 → first static Friend
v0.2.0 → deterministic daily states
v0.3.0 → deterministic moments inside those states
```

The current merged v0.3.0 implementation keeps one Medium Friend and one room as the product boundary, implemented in [`main.jsx`](main.jsx) for ScriptWidget.

The current Friend has `morning`, `coding`, and `sleeping` states derived from device-local time and normalized schedule configuration. Each state has exactly two deterministic Friend moments. All six moments were owner-validated through the Issue #20 integration gate. Release publication and tagging remain separate from implementation state.
...
- Validation overrides: `DEV_OVERRIDE_STATE` and `DEV_OVERRIDE_MOMENT`, outside supported configuration and committed as `null`
...
The current concrete implementation refines that layering without requiring a universal state engine:
...
+ normalized schedule
→ configured Friend state
→ development-only effective-state resolution
→ deterministic Friend moment
→ development-only effective-moment resolution
→ moment dialogue
+ moment visual state
→ ScriptWidget Medium UI
```

Device-local time is the current raw runtime signal. Config is normalized before state derivation, and invalid configuration falls back safely to the complete default schedule. Friend state remains separate from Friend moment, and Friend moment remains a domain concept separate from visual presentation and platform UI. Visual presentation and dialogue consume the same effective moment. Moment selection and dialogue are deterministic rather than random. `DEV_OVERRIDE_STATE` and `DEV_OVERRIDE_MOMENT` are validation-only, not normal user customization features. The supported config surface is intentionally narrow.

## Offline-first and privacy-conscious direction

The current widget makes no network requests and requires no account, backend, storage, or credential. It reads device-local date and hour for state, moment, and dialogue derivation. Moment selection uses no persistence. Future features should prefer offline-first behavior and minimal data collection. Private developer data must not be used without a clear threat model and secure credential strategy.
...
The current moments use color-independent cues such as eye shape, silhouette posture, monitor emphasis, lamp emphasis, and room treatment so that sibling moments are not communicated by color alone. The Friend and room remain the same character and environment across moments.
...
Issues #17 through #19 used focused source-level validation for scoped foundation, visual, and dialogue changes. Issue #20 provided the owner-controlled six-moment device gate before the v0.3.0 behavior was considered ready for documentation. This batching reduces repeated device cycles but does not remove runtime validation for runtime-sensitive milestone behavior.

## Current non-goals

The current v0.3.0 implementation does not include:

- GitHub integration
- weather or calendar integration
- multiple Friendz
- user moment configuration
- moment schedule configuration
